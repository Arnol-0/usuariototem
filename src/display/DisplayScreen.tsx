...
export default function DisplayScreen() {
  ...
  const currentPlayingTicketId = useRef<string | null>(null);
  ...
  const processQueue = async () => {
    ...
    while (announcementsQueue.current.length > 0) {
      ...
      currentPlayingTicketId.current = ticket.id;
      ...
      if (isInitialLoad.current) {
        ...
        if (newAnnouncements.length > 0) {
          for (const ann of newAnnouncements) {
            // Anti-spam: ignorar si ya se está anunciando el mismo turno o si ya está en cola
            if (currentPlayingTicketId.current === ann.ticket.id) continue;
            if (announcementsQueue.current.some(a => a.ticket.id === ann.ticket.id)) continue;
            
            announcementsQueue.current.push(ann);
          }
          if (announcementsQueue.current.length > 0) {
            processQueue();
          }
